...
public class CustomSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler {
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		if (!isAdminAuthority(authentication)) {
			...
			if (org.springframework.util.StringUtils.isEmpty(targetUrl)
					|| org.springframework.util.StringUtils.pathEquals(targetUrl, "/")) {
				targetUrl = "/admin"; // we can build custom logic
			}
	...
	protected boolean isAdminAuthority(final Authentication authentication) {
		return org.springframework.util.CollectionUtils.isEmpty(authentication.getAuthorities())
